-- Migration 029 — Workspace terminology (admin → workspace / owner_user_id)
-- Replaces the failed former 027. Drop raised_by checks BEFORE updating values.

-- ---------------------------------------------------------------------------
-- 1) Drop old raised_by checks (they only allow 'admin' | 'client')
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4) clients.admin_id → owner_user_id (skip if already renamed)
-- ---------------------------------------------------------------------------
...
create index if not exists clients_is_active_idx on public.clients (owner_user_id, is_active);

drop index if exists clients_admin_email_unique;
drop index if exists clients_owner_email_unique;
...
drop policy if exists "Workspace owners own their clients" on public.clients;
...
-- ---------------------------------------------------------------------------
-- 5) client_conversation_messages.author_admin_id → author_user_id
-- ---------------------------------------------------------------------------

drop policy if exists "Admins manage client conversation messages"
  on public.client_conversation_messages;
drop policy if exists "Workspace owners manage client conversation messages"
  on public.client_conversation_messages;
...
begin
  if exists (
    ...
    from information_schema.columns
    where table_schema = 'public'
      and table_name = 'client_conversation_messages'
