...
import type { ReminderTimeSelectProps } from "@/features/reminders/types/components";
import { REMINDER_AVAILABLE_TIMES } from "@/features/reminders/constants/remindersCalendar";
...
export function ReminderTimeSelect({
  selectedTime,
  summaryLabel,
    ...
    <Popover open={open} onOpenChange={setOpen} modal>
      ...
      >
        <div className="shrink-0 border-b border-border px-4 py-3">
          ...
          <p className="mt-1 text-center text-xs text-muted-foreground">{summaryLabel}</p>
        </div>
        <div
          role="listbox"
        ...
        >
          <div className="grid grid-cols-1 gap-2">
            {REMINDER_AVAILABLE_TIMES.map((time) => {
              const isSelected =
