    ...
    <Popover open={open} onOpenChange={handleOpenChange} modal>
      <PopoverTrigger asChild>
        <Button
          ...
          disabled={disabled}
          className={cn(
            "h-auto w-max min-w-40 max-w-none justify-between gap-2 rounded-lg border border-ring/60 bg-card px-3 py-2 text-sm font-medium text-foreground shadow-xs hover:bg-muted/50 dark:border-input dark:bg-muted/40",
            className,
          )}
        >
          <span className="flex w-max min-w-0 flex-nowrap items-center gap-1.5">
            {value.length === 0 ? (
              ...
              value.map((selectedValue) => (
                <span
                  ...
                  className="shrink-0 whitespace-nowrap rounded-md border border-border bg-muted px-2 py-0.5 text-xs font-medium text-foreground"
                >
                  {labelByValue.get(selectedValue) ?? fallbackSelectedLabel}
                </span>
              ))
      ...
      <PopoverContent
        align="start"
        side="bottom"
        className="z-100 flex max-h-80 w-56 min-w-(--radix-popover-trigger-width) flex-col gap-0 overflow-hidden p-0 shadow-2xl"
