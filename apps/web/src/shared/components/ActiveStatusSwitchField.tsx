      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs leading-relaxed text-muted-foreground">
            {checked
              ? `This ${entityLabel} is active and listed in your directory.`
              : `Inactive ${entityLabel}s stay in history but are hidden from the default list.`}
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-2 pt-0.5 text-muted-foreground">
          <span className={cn("text-xs font-semibold", !checked && "text-foreground")}>
            Inactive
          </span>
          <Switch
            checked={checked}
